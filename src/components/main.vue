    ...
    <div>
        <van-sticky>
            <van-nav-bar class="bar_sty" left-text="论坛">
                <template #right>
                    ...
                    <van-popup v-model:show="show" position="top" :style="{ padding: '0px' }">
                        <van-search v-model="searchContent" shape="round" background="#4fc08d" placeholder="请输入搜索关键词"
            ...
            <div class="title_menu">
                ...
                <van-button class="smallBtn" size="small" @click="sortbyid">
                <div v-if="newflag">
                    最早
                </div>
                <div v-else>
    ...
    methods: {
        ...
        sortbyid() {
            if (this.newflag == false) {
                let newList = [];
                newList = this.post.sort((a, b) => a.id < b.id ? 1 : a.id > b.id ? -1 : 0);
                this.sort=newList
                console.log(this.post)
                this.newflag = true
            }else
            {   this.$store.state.post = JSON.parse(localStorage.getItem("postInfo"))
                this.post = this.$store.state.post
                console.log(this.$store.state.post)
                console.log(this.post)
                this.newflag = false
